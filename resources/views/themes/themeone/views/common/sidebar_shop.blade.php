 	...
 	<form enctype="multipart/form-data" name="filters" method="get">
        ...
        <div class="card">
          ...
          if(!empty($result['filter_attribute']['option_values']) and in_array($values['value_id'],$result['filter_attribute']['option_values'])) print 'checked';
            ...
            @endforeach
            @endif
             @if(count($result['filters']['brand'])>0)
               
            <div class="block">
                    <div class="card-title @if(count($result['filters']['brand'])==$key+1) last @endif">Brand</div>
                    <div class="card-body">
                        <ul class="list">
                            @foreach($result['filters']['brand'] as $key=>$attr_data)
                            <li>
                                <div class="form-check">
           <label class="form-check-label"> 
            <input class="form-check-input filters_box" name="brand[]" type="checkbox" value="{{$attr_data->manufacturers_name}}" <?php if(!empty($result['filter_attribute']['brand']) and in_array($attr_data->manufacturers_id,$result['filter_attribute']['brand'])) print 'checked';?>>
            {{$attr_data->manufacturers_name}}
           </label> 
