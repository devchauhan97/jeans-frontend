    ...
    <ul class="navbar-nav flex-column">
     @foreach($result['commonContent']['categories'] as $categories_data)
      ...
      <li class="nav-item dropdown">
        <a href="{{ URL::to('/shop')}}?category={{$categories_data->categories_slug}}" class="nav-link dropdown-toggle">
          <img class="img-fuild" src="{{asset('').$categories_data->icon}}">{{$categories_data->categories_description->categories_name}} @if(count($categories_data->sub_categories)>0) <i class="fa fa-angle-right " aria-hidden="true"></i> @endif
        </a>
        ...
        <ul class="dropdown-menu multi-level">
            ...
            <li class="dropdown-submenu">
              <a  class="dropdown-item" tabindex="-1" href="{{ URL::to('/shop')}}?category={{$sub_categories_data->categories_slug}}">
                <img class="img-fuild" src="{{asset('').$sub_categories_data->sub_icon}}">
                {{$sub_categories_data->categories_description->categories_name}}
              </a>              
            </li>            
            @endforeach 
          </ul>
