		...
		jQuery.ajax({			
			...
			success: function (res) {			
				...
				if(obj.success==0){
					...
					jQuery(selector).find('i').removeClass('fa-heart-o');
					jQuery(selector).find('i').addClass('fa-heart');
					jQuery(selector).find('i').children('span').html(obj.total_likes);
					jQuery('#wishlist-count').html(parseInt(user_count)+ parseInt(1));	
					jQuery(selector).find('i').children('.badge').html(obj.total_likes);
				}else if(obj.success==1){
					jQuery(selector).find('i').removeClass('fa-heart');
					jQuery(selector).find('i').addClass('fa-heart-o');
...
jQuery(document).on('click', '.update-single-Cart', function(e){	
	...
	jQuery( function() {		 
		...
		var min_price_show = jQuery( "#min_price_mobile" ).val();
		var max_price_show = jQuery( "#max_price_mobile" ).val() ;
		...
		jQuery( "#slider-range-mobile" ).slider({
			...
			values: [ min_price_show, max_seleted ],
			slide: function( event, ui ) {
				jQuery('#min_price_mobile').val(ui.values[ 0 ] );
				jQuery('#max_price_mobile').val(ui.values[ 1 ] );
			   
				jQuery('#min_price_show_mobile').val( ui.values[ 0 ] );
				jQuery('#max_price_show_mobile').val( ui.values[ 1 ] );
			},
			create: function(event, ui){
				jQuery(this).slider('value',20);
			}
		});	   
		jQuery( "#min_price_show_mobile" ).val( jQuery( "#slider-range-mobile" ).slider( "values", 0 ) );	   
		jQuery( "#max_price_show_mobile" ).val(jQuery( "#slider-range-mobile" ).slider( "values", 1 ) );
		//jQuery( "#slider-range" ).slider( "option", "max", 50 );
	});
