...
class NewsController extends DataController
{
	...
	public function getNewsCategories(){		
		...
		$data 		=	array();				
		$categories = NewsCategory::LeftJoin('news_categories_description', 'news_categories_description.categories_id', '=', 'news_categories.categories_id')
		...
		if(count($categories)>0){			
			foreach($categories as $categories_data){				
				...
				$news = NewsCategory::LeftJoin('news_to_news_categories', 'news_to_news_categories.categories_id', '=', 'news_categories.categories_id')
	...
	public function news(Request $request){
		...
		if(!empty($request->category) and $request->category!='all'){
			...
			$categories = NewsCategory::leftJoin('news_categories_description','news_categories_description.categories_id','=','news_categories.categories_id')->where('news_categories_slug',$category)->where('news_categories_description.language_id',Session::get('language_id'))->get();
	...
	public function getAllNews($data){
		...
		$categories = NewsToNewsCategory::LeftJoin('news', 'news.news_id', '=', 'news_to_news_categories.news_id');
	...
	public function newsdetail(Request $request){
		...
		$news =News::leftjoin('news_description','news_description.news_id','=','news.news_id')
